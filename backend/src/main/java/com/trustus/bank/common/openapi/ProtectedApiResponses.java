// Reusable OpenAPI error responses for authenticated endpoints.
...
@ApiResponses({
        ...
        @ApiResponse(
                responseCode = "401",
                description = "Missing or invalid JWT — call without Authorize, or use an expired token"
        ),
        @ApiResponse(
                responseCode = "403",
                description = "Forbidden — wrong role, or unapproved customer on /api/customer/** and /api/atm/**",
                content = @Content(
                        mediaType = "application/json",
                        examples = @ExampleObject(name = "Pending approval", value = OpenApiExamples.PENDING_APPROVAL_ERROR)
                )
        ),
        @ApiResponse(
                responseCode = "404",
...
public @interface ProtectedApiResponses {
}
